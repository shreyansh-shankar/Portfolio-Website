...
const Stage = ({ icon: Icon, label, delay }: { icon: any, label: string, delay: number }) => (
  ...
  >
    <div className="w-16 h-16 rounded-xl bg-[#111] border border-gray-700 flex items-center justify-center text-indigo-400 shadow-[0_0_15px_rgba(99,102,241,0.2)] hover:border-indigo-500 hover:shadow-[0_0_25px_rgba(99,102,241,0.4)] transition-all">
      <Icon size={28} />
    </div>
...
const Connection = ({ delay }: { delay: number }) => (
  <motion.div
    ...
    viewport={{ once: true }}
    className="h-0.5 bg-gradient-to-r from-gray-800 via-indigo-900 to-gray-800 flex-1 mx-2 origin-left relative"
  >
    <motion.div 
      animate={{ x: [-10, 100] }}
      transition={{ duration: 2, repeat: Infinity, ease: "linear" }}
      className="absolute top-1/2 -translate-y-1/2 w-8 h-1 bg-indigo-500 blur-sm rounded-full opacity-50"
    />
  </motion.div>
);
...
const DevOpsPipeline: React.FC = () => {
  return (
    <div className="w-full py-12 px-4 bg-[#080808] border-y border-gray-900 relative overflow-hidden">
      {/* Background Grids */}
      ...
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-12">
          <h3 className="text-2xl font-mono text-gray-300 mb-2">
            <span className="text-indigo-500">{'>'}</span> ARCHITECTURE_FLOW
          </h3>
